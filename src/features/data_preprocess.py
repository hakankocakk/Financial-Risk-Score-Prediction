...
def ordinalencoding(dataframe : pd.DataFrame, model_path : str, train=True) -> pd.DataFrame:
...
def onehotencoding(dataframe : pd.DataFrame, model_path : str, train=True) -> pd.DataFrame:
...
def normalization(dataframe : pd.DataFrame, model_path : str, train=True) -> pd.DataFrame:
...
def main():
    ...
    try:
        train = load_data(os.path.join(interim_data_path, "train.csv"))
        validation = load_data(os.path.join(interim_data_path, "validation.csv"))
        test = load_data(os.path.join(interim_data_path, "test.csv"))
        ...
        save_data(validation_data, os.path.join(processed_data_path, "validation.csv"))
